...
def load_stats() -> dict:
    ...
    if not os.path.exists(STATS_FILE):
        # Создаём файл с пустой статистикой
        default_stats = {
            "users": {},
            "total": {
                "kuni": 0,
...
def update_user_stats(user_id: int, user_name: str, action: str, target_id: int = None):
    ...
    if user_id_str not in stats["users"]:
        ...
        # Обновляем имя (на случай если сменилось)
    ...
    # Если есть цель, обновляем и её статистику (получатель)
...
async def cmd_start(message: Message):
    ...
    await message.answer(
        "🍆 Привет! Я бот для взрослых команд.\n\n"
        ...
        "💡 Просто ответь на сообщение командой!",
        parse_mode="HTML"
    )


@dp.message(F.text == "куни")
...
@dp.message(F.text == "топ куни")
async def cmd_top_kuni(message: Message):
    """Топ по куни."""
    stats = load_stats()
    
    # Сортируем пользователей по количеству куни
    users_list = []
...
@dp.message(F.text == "топ выпивки")
async def cmd_top_drinker(message: Message):
    """Топ по выпивке (одиночка + компания)."""
    stats = load_stats()
