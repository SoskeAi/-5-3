...
async def cmd_top_drinker(message: Message):
    ...
    for i, u in enumerate(top, 1):
        if i == 1:
            text += f"🥇 {u['name']} — {u['total']} 🍺 (один: {u['alone']}, в компании: {u['with']})\n"
        elif i == 2:
            text += f"🥈 {u['name']} — {u['total']} 🍺 (один: {u['alone']}, в компании: {u['with']})\n"
        elif i == 3:
            text += f"🥉 {u['name']} — {u['total']} 🍺 (один: {u['alone']}, в компании: {u['with']})\n"
        else:
            text += f"{i}. {u['name']} — {u['total']} 🍺 (один: {u['alone']}, в компании: {u['with']})\n"
...
@dp.message(F.text.func(lambda text: text and text.lower() == "вся стата"))
